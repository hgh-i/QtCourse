...
void QWMainWind::about(){
 QDialog dialog(this);
 dialog.setWindowTitle("关于");
 dialog.setModal(true);
 dialog.resize(300, 200); // 主布局
 QHBoxLayout *layout = new QHBoxLayout(&dialog); // 左侧：图片（ QLabel + QPixmap ）
 QLabel *imageLabel = new QLabel; QPixmap pixmap(":/icons/avatar.png"); // 👈 替换为你自己的图片路径 // 可以设置图片大小
 imageLabel->setPixmap(pixmap.scaled(64, 64, Qt::KeepAspectRatio, Qt::SmoothTransformation));
 imageLabel->setAlignment(Qt::AlignCenter); // 右侧：文字信息
 QLabel *textLabel = new QLabel; textLabel->setText(  "作者：黄千铧\n" "学号：2023414290216\n"  );
 textLabel->setAlignment(Qt::AlignVCenter);
 textLabel->setWordWrap(true);
 // 添加到布局
 layout->addWidget(imageLabel);
 layout->addWidget(textLabel); // 显示对话框
 dialog.exec();
}
